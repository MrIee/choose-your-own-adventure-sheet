<template>
  <div class="tw:w-full tw:md:w-1/2 box">
    ...
    <div v-if="isAddButtonVisible" class="tw:flex tw:mt-auto">
      <button @click="onClickAddEncounter">Add Encounter</button>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    onClickRemove(): void {
      this.$emit('remove-encounter');
    },
    onClickAddEncounter() {
      this.$emit('add-encounter');
    },
  },
});
</script>
